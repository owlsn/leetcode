...
public class romanToInt {
    ...
    public int romanToInt(String s) {
        ...
        for(int i = 0; i < s.length(); i ++){
            if(i + 2 <= s.length() && nums.containsKey(s.substring(i, i + 2))){
                result += nums.get(s.substring(i, i + 2));
                //移动两个字符
                i ++;
                continue;
            }
            else if(i + 1 <= s.length() && nums.containsKey(s.substring(i, i + 1))){
                result += nums.get(s.substring(i, i + 1));
            }
        }
        return result;
    }
